...
vector<string> setparser_queries {
...
vector<const MySQLParser::AstNode*> ext_vars_assigns(const MySQLParser::AstNode* root) {
	...
	vector<pair<const MySQLParser::AstNode*, size_t>> queue {{ root, 0 }};
	...
	if (node->type == NodeType::NODE_SET_STATEMENT) {
		...
		for (const auto& v : vars_assings) {
			...
			if (!check_sys_var(v).second) {
				...
				const auto var_it {
					std::find_if(tracked_vars.begin(), tracked_vars.end(),
						[&sys_var] (const auto& str) -> bool {
							return strcasecmp(sys_var.second->value.c_str(), str.c_str()) == 0;
						}
					)
				};

				if (var_it == tracked_vars.end() && sys_var.second->value != "autocommit") {
					return false;
				}
			}
		}
...
string rm_outer_paren(const string& s) {
	if (s.size() < 2) {
		return s;
	} else {
		if (s.front() == '(' && s.back() == ')') {
...
string acc_child(MySQLParser::AstNode* c, const string& s) {
	const string res { "(" + to_string(c->type) + "," + c->value + ")" };
...
pair<bool,string> verf_sql_mode_val(const string& subexpr) {
	...
	std::unique_ptr<MySQLParser::AstNode> ast { parser.parse(rm_outer_paren(subexpr)) };
		...
		if (rc_child.first) {
			...
			if (type == NodeType::NODE_EXPR) {
				...
				const auto valid_select_subexpr = [] (const string& q, const MySQLParser::AstNode* c) -> pair<bool,string> {
					if (c->type != NodeType::NODE_SELECT_RAW_SUBQUERY) {
						...
						std::unique_ptr<MySQLParser::AstNode> ast {
							parser.parse(rm_outer_paren(sub_select))
						};
				...
				const auto verf_select_item = [&] (const MySQLParser::AstNode* n) -> pair<bool,string> {
					...
					std::deque<const MySQLParser::AstNode*> n_queue { n };
	...
	for (const auto& q : test_qs.second) {
		...
		if (ast) {
			...
			for (auto va : vars_assigns) {
				...
				const string d_val {
					is_id || is_str ?  fold(fold_spaces, ext_inner_val(p_val)) : p_val
				};
				const string f_val {
					std::all_of(d_val.begin(), d_val.end(), [] (char c) { return is_space_char(c); }) ?
						"" : d_val
				};
				...
				const vector<string> re_vals { regex_vals[low_name] };
